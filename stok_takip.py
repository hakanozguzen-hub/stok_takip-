...
# ==============================================================================
# 💾 SQLITE VERİTABANI MOTORU (TAKILMALARI ÖNLER)
# ==============================================================================
DB_YOLU = "mayra_stok_veritabani_son.db"
...
# ==============================================================================
# 🎨 GÖRSEL AYARLAR (SADECE STANDART OKUNAKLI BUTON GENİŞLİĞİ)
# ==============================================================================
st.set_page_config(page_title="Stok Takip Sistemi", layout="wide")
st.markdown("""
    <style>
    div.stButton > button { width: 100%; font-weight: bold; padding: 10px; }
    </style>
...
with st.expander(GIRIS_PANEL_UST_YAZI, expanded=True):
    ...
    if st.button(GIRIS_KAYDET_BUTON_METNI, key="btn_g_ekle"):
        if g_kod and g_aciklama and g_firma:
            ...
            cursor.execute("INSERT INTO girisler (stok_kodu, date_ver, firma, adet) VALUES (?, ?, ?, ?)" if "date_ver" in str(e) else "INSERT INTO girisler (stok_kodu, tarih, firma, adet) VALUES (?, ?, ?, ?)", (g_kod, g_tarih, g_firma, g_adet))
            conn.commit()
            conn.close()
...
# ==============================================================================
# 📊 ALT BÖLÜM - TÜM RAPORLAMA, FİLTRELEME VE SİLME MOTORU (TAŞMAZ VE KIRILMAZ)
# ==============================================================================
st.markdown("---")
st.header(YONETIM_UST_YAZI)

arama_sorgusu = st.text_input("🔍 Bulmak istediğiniz Stok Kodunu veya Ürün Adını yazın:", "", key="arama_ana_mudahale").strip().lower()
